...
func (r *AuditRepo) CreateNoTx(ctx context.Context, actorUserID *uuid.UUID, action, entityType string, entityID uuid.UUID, metaJSON []byte, ip string) error {
	if metaJSON == nil {
		metaJSON = []byte(`{}`)
	}
...
func (r *AuditRepo) List(ctx context.Context, action string, limit, offset int) ([]models.AuditLog, int, error) {
	var total int
	if action != "" {
		if err := r.db.GetContext(ctx, &total, `SELECT COUNT(*) FROM audit_logs WHERE action = $1`, action); err != nil {
	...
	var err error
	if action != "" {
		err = r.db.SelectContext(ctx, &logs, query+`WHERE al.action = $1 ORDER BY al.created_at DESC LIMIT $2 OFFSET $3`, action, limit, offset)
	} else {
		err = r.db.SelectContext(ctx, &logs, query+`ORDER BY al.created_at DESC LIMIT $1 OFFSET $2`, limit, offset)
	}
	if err != nil {
